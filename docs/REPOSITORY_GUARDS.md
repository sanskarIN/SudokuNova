...
A new path fails closed when it is outside the maintained taxonomy. This makes "document every file" an enforceable repository invariant rather than a static checklist that becomes stale as files are added or moved.
...
`--verbose` prints one ownership line for every tracked file. `--markdown` emits a deterministic per-file table suitable for review/evidence capture.

The complete area-to-document mapping and maintenance rules are defined in [Repository File Reference and Documentation Coverage](REPOSITORY_FILE_REFERENCE.md).
...
A passing coverage guard proves that every tracked path has documentation ownership. It does not prove the prose is factually current, nor does it replace build/test/manual/release evidence.
...
When adding, moving, renaming, or deleting repository files, run both the documentation-coverage and Markdown-link guards. If a new path family is intentional, add a narrow coverage rule and regression test rather than disguising it under an unrelated area.
