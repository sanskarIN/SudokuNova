...
object GameStateCodec {
    ...
    private fun decodeV4(parts: List<String>): GameState {
        ...
        val replay = parts[16].takeIf(String::isNotBlank)?.toLong()
        require(replay == null || replay > 0L)
        val challengeType = parts[17].takeIf(String::isNotBlank)
    ...
    private fun baseState(
        ...
        elapsedSeconds = parts[elapsedIndex].toLong().also { require(it >= 0L) },
        mistakes = parts[mistakesIndex].toInt().also { require(it >= 0) },
        hintsUsed = parts[hintsIndex].toInt().also { require(it >= 0) },
