...
interface ChallengeResultDao {
    @Query("SELECT * FROM challenge_results ORDER BY completedAtEpochMillis DESC, id DESC")
    fun observeAll(): Flow<List<ChallengeResultEntity>>

    @Query("SELECT * FROM challenge_results WHERE challengeType = :challengeType ORDER BY completedAtEpochMillis DESC, id DESC")
    fun observeType(challengeType: String): Flow<List<ChallengeResultEntity>>

    @Query("SELECT * FROM challenge_results WHERE challengeType = :challengeType AND challengeKey = :challengeKey LIMIT 1")
